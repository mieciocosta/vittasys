async function renderAgenda() {
  ...
  function findReg(bairro) {
    if (!bairro) return null;
    const bl = bairro.toLowerCase().trim();
    for (const r of regioes) {
      if ((r.bairros || []).some((b) => b.toLowerCase().trim() === bl)) return r;
    }
    return null;
  }
  ...
  async function draw() {
    ...
    const title =
      view === 'mes'
        ? `${MESES[sel.getMonth()]} ${sel.getFullYear()}`
        : view === 'semana'
          ? `Semana — ${fB(sel)}`
          : `${DF[sel.getDay()]}, ${fB(sel)}`;
    ...
    if (view !== 'mes') {
      left.appendChild(h('button', { className: 'btn btn-outline btn-sm', onClick: () => { view = 'mes'; draw(); } }, '← Calendário'));
    }
    ...
    if (AppState.isMaster()) {
      ra.appendChild(h('button', { className: 'btn btn-primary btn-sm', onClick: () => modalGerar() }, '⚡ Gerar'));
    ...
    nav.appendChild(h('button', {
      ...
      style: 'width:30px',
      onClick: () => {
        if (step === 'm') sel.setMonth(sel.getMonth() - 1);
        else sel.setDate(sel.getDate() - step);
        draw();
      }
    }, '‹'));

    nav.appendChild(h('button', {
      className: `btn btn-sm ${isH(sel) ? 'btn-primary' : 'btn-outline'}`,
      style: 'font-size:11px',
      onClick: () => { sel = new Date(); sel.setHours(0, 0, 0, 0); draw(); }
    }, 'Hoje'));
    ...
    rc.appendChild(h('button', {
      className: `btn btn-sm ${!regF ? 'btn-primary' : 'btn-ghost'}`,
    ...
    regioes.forEach((rg) => {
      rc.appendChild(h('button', {
        className: `btn btn-sm ${regF == rg.id ? 'btn-primary' : 'btn-ghost'}`,
        style: `font-size:9px;padding:3px 7px;border-left:3px solid ${rg.cor}`,
        onClick: () => { regF = String(rg.id); draw(); }
      }, rg.nome));
    });
  ...
  async function drawMes() {
    ...
    const p = { mes: `${y}-${String(m + 1).padStart(2, '0')}` };
    if (regF) p.regiao_id = regF;

    const items = await Api.agendaList(p) || [];
    const byD = {};
    items.forEach((it) => {
      const k = fI(new Date(it.data));
      if (!byD[k]) byD[k] = [];
      byD[k].push(it);
    });
    ...
    for (let i = 0; i < startDow - 1; i++) {
      grid.appendChild(h('div', { style: 'min-height:80px;border:1px solid #f1f5f9;background:#fafafa' }));
    }
    ...
    for (let d = 1; d <= last.getDate(); d++) {
      ...
      const cell = h('div', {
        style: `min-height:80px;border:1px solid ${hoje ? 'var(--primary)' : '#f1f5f9'};padding:3px;cursor:pointer;${hoje ? 'background:#f0fffe;' : ''}`,
      ...
      top.appendChild(h('span', {
        style: `font-size:${hoje ? '13px' : '11px'};font-weight:${hoje ? '800' : '600'};color:${hoje ? 'var(--primary)' : 'var(--text-2)'}`
      }, String(d)));
      ...
      dayIt.slice(0, 3).forEach((it) => {
        cell.appendChild(h('div', {
          style: `font-size:8px;padding:1px 3px;margin-bottom:1px;border-radius:2px;background:${SC[it.status]}15;color:${SC[it.status]};overflow:hidden;white-space:nowrap;text-overflow:ellipsis`
    ...
    const tc = startDow - 1 + last.getDate();
    const rem = 7 - (tc % 7);
    if (rem < 7) {
      for (let i = 0; i < rem; i++) {
        grid.appendChild(h('div', { style: 'min-height:80px;border:1px solid #f1f5f9;background:#fafafa' }));
    ...
    const tot = items.length,
      re = items.filter((i) => i.status === 'realizado').length,
      co = items.filter((i) => i.status === 'confirmado').length,
      fa = items.filter((i) => i.status === 'faltou').length;
    ...
    [['📋', tot - re - fa - co, 'Pend.', '#3b82f6'], ['✓', co, 'Conf.', '#2BBCB3'], ['✅', re, 'Feitos', '#059669'], ['✗', fa, 'Faltas', '#dc2626'], ['📊', tot, 'Total', 'var(--navy)']].forEach(([ic, v, l, c]) => {
      sm.appendChild(h('div', { style: `padding:6px 10px;text-align:center;flex:1;border-radius:8px;background:${c}08` },
        h('span', { style: `font-size:15px;font-weight:800;color:${c}` }, String(v)),
        h('span', { style: 'font-size:9px;color:var(--text-3);margin-left:4px' }, l)
      ));
    });
  ...
  async function drawSemana() {
    ...
    for (let i = 0; i < 7; i++) {
      ...
      days.push({ date: dt, iso: fI(dt), items: items.filter((it) => fI(new Date(it.data)) === fI(dt)) });
    }
    ...
    days.forEach((d) => {
      ...
      grid.appendChild(h('div', {
        style: `padding:6px;text-align:center;${hj ? 'background:var(--primary);color:white' : 'background:var(--navy);color:white'};cursor:pointer`,
    ...
    for (let row = 0; row < mx; row++) {
      days.forEach((d) => {
        const it = d.items[row];
        if (!it) {
          grid.appendChild(h('div', { style: 'padding:4px;min-height:48px;border:1px solid #f1f5f9' }));
          return;
        }

        const cell = h('div', {
          style: `padding:5px;border:1px solid #f1f5f9;background:${SC[it.status]}12;cursor:pointer;min-height:48px`,
          onClick: () => { sel = new Date(d.date); view = 'dia'; draw(); }
        });
  ...
  async function drawDia() {
    ...
    items.forEach((i) => { if (sts[i.status] != null) sts[i.status]++; });
    ...
    [['📋', sts.agendado, 'Agend.', '#3b82f6'], ['✓', sts.confirmado, 'Conf.', '#2BBCB3'], ['✅', sts.realizado, 'Feitos', '#059669'], ['✗', sts.faltou, 'Faltas', '#dc2626']].forEach(([ic, v, l, c]) => {
      sb.appendChild(h('div', {
        style: `padding:6px 10px;background:${c}10;border-left:3px solid ${c};border-radius:8px;flex:1;display:flex;align-items:center;gap:6px`
      },
      h('span', { style: `font-size:15px;font-weight:800;color:${c}` }, String(v)),
      h('span', { style: 'font-size:9px;color:var(--text-3)' }, l)));
    });

    sb.appendChild(h('button', {
      className: 'btn btn-primary btn-sm',
      style: 'height:auto;white-space:nowrap',
      onClick: () => modalCriar(fI(sel))
    }, '+ Novo'));

    wrap.appendChild(sb);

    if (!items.length) {
      wrap.appendChild(h('div', { style: 'text-align:center;padding:40px;color:var(--text-3)' },
        h('div', { style: 'font-size:36px' }, '📅'),
        h('div', { style: 'font-size:14px;font-weight:600;margin-top:8px' }, 'Nenhum agendamento')
      ));
      return;
    }
    ...
    items.forEach((it) => {
      ...
      if (seen[key]) {
        seen[key].vacinas.push({ nome: it.vacina, dose: it.dose_numero, id: it.id });
      } else {
        seen[key] = { ...it, vacinas: [{ nome: it.vacina, dose: it.dose_numero, id: it.id }] };
    ...
    merged.forEach((i) => {
      ...
      if (!groups[k]) groups[k] = { cor: i.regiao_cor || '#94a3b8', items: [] };
    ...
    Object.entries(groups).forEach(([rn, gr]) => {
      ...
      sec.appendChild(h('div', {
        style: `padding:7px 12px;background:${gr.cor}15;font-weight:700;font-size:12px;color:${gr.cor};display:flex;justify-content:space-between`
      }, h('span', null, '📍 ' + rn), h('span', null, gr.items.length + ' atend.')));

      gr.items.forEach((it) => {
        const card = h('div', {
          style: 'display:flex;align-items:center;gap:8px;padding:8px 12px;border-bottom:1px solid #f1f5f9;cursor:pointer',
          onClick: () => { AppState._returnTo = 'agenda'; AppState.verCliente(it.cliente_id); }
        });

        card.appendChild(h('div', {
          style: 'font-size:14px;font-weight:800;color:var(--navy);min-width:42px;text-align:center'
        }, esc(it.horario || '--:--')));

        card.appendChild(h('div', {
          style: `width:8px;height:8px;border-radius:50%;background:${SC[it.status]};flex-shrink:0`
        }));
        ...
        if (it.endereco || it.bairro) {
          links.appendChild(h('a', {
            href: `https://www.google.com/maps/search/${encodeURIComponent((it.endereco || it.bairro) + ', São Luís MA')}`,
            target: '_blank',
        ...
        if (it.celular) {
          links.appendChild(h('a', {
            href: `https://wa.me/55${(it.celular || '').replace(/\D/g, '')}`,
            target: '_blank',
            style: 'font-size:14px;text-decoration:none',
            onClick: (e) => e.stopPropagation()
          }, '📱'));
        ...
        if (it.status === 'agendado') {
          acts.appendChild(h('button', {
            style: 'border:none;background:#2BBCB320;border-radius:6px;padding:4px 8px;font-size:13px;cursor:pointer',
            title: 'Confirmar',
            onClick: async (e) => {
              e.stopPropagation();
              for (const v of it.vacinas) await Api.agendaStatus(v.id, { status: 'confirmado' });
              draw();
            }
          }, '✓'));
        }
        ...
        if (it.status === 'confirmado') {
          acts.appendChild(h('button', {
            style: 'border:none;background:#05966920;border-radius:6px;padding:4px 8px;font-size:13px;cursor:pointer',
            title: 'Realizado',
            onClick: async (e) => {
              e.stopPropagation();
              for (const v of it.vacinas) await Api.agendaStatus(v.id, { status: 'realizado' });
              draw();
            }
          }, '✅'));

          acts.appendChild(h('button', {
            style: 'border:none;background:#dc262620;border-radius:6px;padding:4px 8px;font-size:13px;cursor:pointer',
            title: 'Faltou',
            onClick: async (e) => {
              e.stopPropagation();
              for (const v of it.vacinas) await Api.agendaStatus(v.id, { status: 'faltou' });
              draw();
            }
          }, '✗'));
        }
        ...
        if (['agendado', 'confirmado'].includes(it.status)) {
          acts.appendChild(h('button', {
            style: 'border:none;background:var(--bg-subtle);border-radius:6px;padding:4px 8px;font-size:12px;cursor:pointer',
            title: 'Editar',
            onClick: (e) => { e.stopPropagation(); modalEdit(it); }
          }, '✏️'));
        }
  ...
  function modalCriar(dateISO) {
    showModal('+ Novo Agendamento', async (body, close) => {
      const fd = { data: dateISO, horario: '09:00', cliente_id: '', vacina_ids: [], regiao_id: '', observacoes: '', usuario_id: AppState.usuario?.id };
      const cs = h('div', { style: 'margin-bottom:18px' });
      cs.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '👤 PACIENTE (apenas com plano ativo)'));
      ...
      ci.addEventListener('input', () => {
        ...
        timer = setTimeout(async () => {
          ...
          if (!res.length) {
            cl.innerHTML = '<div style="padding:10px;color:var(--text-3);font-size:12px">Nenhum cliente com plano ativo encontrado</div>';
            return;
          }

          res.forEach((c) => {
            cl.appendChild(h('div', {
              ...
              onClick: () => {
                ...
                if (autoR) {
                  fd.regiao_id = autoR.id;
                  regDiv.querySelectorAll('[data-rid]').forEach((b) => {
                    b.className = b.dataset.rid == autoR.id ? 'btn btn-sm btn-primary' : 'btn btn-sm btn-outline';
                  });
                }

                if (c.endereco || c.bairro) {
                  addrInp.value = c.endereco || c.bairro || '';
                  fd.endereco = addrInp.value;
                }
            ...
            h('div', { style: 'display:flex;justify-content:space-between' },
              ...
              h('span', { style: 'font-size:10px;color:var(--text-4)' }, esc(c.codigo_cliente || ''))
            )));
          });
        }, 300);
      });
      ...
      cs.appendChild(ci);
      cs.appendChild(cl);
      cs.appendChild(selBox);
      body.appendChild(cs);

      const row = h('div', { style: 'display:grid;grid-template-columns:1fr 1fr 1fr;gap:12px;margin-bottom:18px' });

      const d1 = h('div');
      d1.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '📅 DATA'));
      const di = h('input', { className: 'input', type: 'date', value: fd.data, style: 'font-size:14px;padding:10px' });
      di.addEventListener('change', (e) => { fd.data = e.target.value; });
      d1.appendChild(di);
      row.appendChild(d1);

      const d2 = h('div');
      d2.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '🕐 HORÁRIO'));
      const hi = h('input', { className: 'input', type: 'time', value: fd.horario, style: 'font-size:14px;padding:10px' });
      hi.addEventListener('change', (e) => { fd.horario = e.target.value; });
      d2.appendChild(hi);
      row.appendChild(d2);

      const d3 = h('div');
      d3.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '📍 TIPO'));
      fd.tipo = 'domiciliar';

      const btnDom = h('button', {
        type: 'button',
        className: 'btn btn-sm btn-primary',
        style: 'flex:1;font-size:10px;padding:8px',
        onClick: () => {
          fd.tipo = 'domiciliar';
          btnDom.className = 'btn btn-sm btn-primary';
          btnClinic.className = 'btn btn-sm btn-outline';
          addrSec.style.display = 'block';
        }
      }, '🏠 Domicílio');

      const btnClinic = h('button', {
        type: 'button',
        className: 'btn btn-sm btn-outline',
        style: 'flex:1;font-size:10px;padding:8px',
        onClick: () => {
          fd.tipo = 'clinica';
          btnClinic.className = 'btn btn-sm btn-primary';
          btnDom.className = 'btn btn-sm btn-outline';
          addrSec.style.display = 'none';
        }
      }, '🏥 Clínica');

      d3.appendChild(h('div', { style: 'display:flex;gap:4px' }, btnDom, btnClinic));
      row.appendChild(d3);
      ...
      addrSec.appendChild(addrInp);
      body.appendChild(addrSec);

      const vd = h('div', { style: 'margin-bottom:18px' });
      vd.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '💉 VACINAS (selecione uma ou mais)'));
      ...
      vacCache.forEach((v) => {
        const btn = h('button', {
          ...
          onClick: () => {
            const idx = fd.vacina_ids.indexOf(v.id);
            if (idx >= 0) {
              fd.vacina_ids.splice(idx, 1);
              btn.className = 'btn btn-sm btn-outline';
            } else {
              fd.vacina_ids.push(v.id);
              btn.className = 'btn btn-sm btn-primary';
            }
            selCount.textContent = fd.vacina_ids.length + ' selecionada(s)';
          }
        }, v.nome);
        vGrid.appendChild(btn);
      });
      vd.appendChild(vGrid);
      body.appendChild(vd);
      ...
      regioes.forEach((r) => {
        regHint.innerHTML += `<span style="color:${r.cor};font-weight:600">${r.nome}</span>: ${(r.bairros || []).slice(0, 5).join(', ')}${(r.bairros || []).length > 5 ? '...' : ''} · `;
      });
      ...
      regioes.forEach((r) => {
        const btn = h('button', {
          type: 'button',
          'data-rid': r.id,
          className: `btn btn-sm ${fd.regiao_id == r.id ? 'btn-primary' : 'btn-outline'}`,
          style: `font-size:11px;padding:5px 10px;border-left:3px solid ${r.cor}`,
          onClick: (e) => {
            fd.regiao_id = r.id;
            rBtns.querySelectorAll('[data-rid]').forEach((b) => { b.className = 'btn btn-sm btn-outline'; });
            e.currentTarget.className = 'btn btn-sm btn-primary';
          }
        }, r.nome);
        rBtns.appendChild(btn);
      });
      regDiv.appendChild(rBtns);
      body.appendChild(regDiv);

      const od = h('div', { style: 'margin-bottom:18px' });
      od.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '💬 OBSERVAÇÕES'));
      const oi = h('input', { className: 'input', style: 'font-size:13px;padding:10px', placeholder: 'Notas para o vacinador...' });
      oi.addEventListener('input', (e) => { fd.observacoes = e.target.value; });
      od.appendChild(oi);
      body.appendChild(od);
      ...
      body.appendChild(h('button', {
        ...
        onClick: async () => {
          ...
          if (r?.success) {
            Toast.show(r.message || 'Agendamento(s) criado(s)!');
            close();
            draw();
          } else {
            Toast.show(r?.error || 'Erro', 'error');
          }
        }
      }, '✓ Agendar'));
    }, '600px');
  ...
  function modalEdit(ag) {
    showModal('✏️ Editar Agendamento', async (body, close) => {
      ...
      const vd = h('div', { style: 'margin-bottom:18px' });
      vd.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '💉 ALTERAR VACINA'));
      ...
      const keepBtn = h('button', {
        ...
        onClick: (e) => {
          fd.vacina_id = '';
          vGrid.querySelectorAll('[data-vid]').forEach((b) => { b.className = 'btn btn-sm btn-outline'; });
          e.currentTarget.className = 'btn btn-sm btn-primary';
        }
      }, `✓ Manter: ${ag.vacina}`);
      vGrid.appendChild(keepBtn);
      ...
      const rd = h('div', { style: 'margin-bottom:18px' });
      rd.appendChild(h('label', { className: 'label', style: 'font-size:13px' }, '📝 MOTIVO DA ALTERAÇÃO *'));
      const ri = h('input', { className: 'input', style: 'font-size:13px;padding:10px', placeholder: 'Obrigatório ao alterar vacina ou data...' });
      ri.addEventListener('input', (e) => { fd.observacoes = e.target.value; });
      rd.appendChild(ri);
      body.appendChild(rd);

      const bt = h('div', { style: 'display:flex;gap:10px' });

      bt.appendChild(h('button', {
  ...
  function modalGerar() {
    showModal('⚡ Gerar Agenda Automática', async (body, close) => {
      const now = new Date();
      ...
      MESES.forEach((m, i) => {
        const btn = h('button', {
          type: 'button',
          className: `btn btn-sm ${fd.mes === i + 1 ? 'btn-primary' : 'btn-outline'}`,
          style: 'font-size:11px;padding:6px',
          onClick: (e) => {
            fd.mes = i + 1;
            mGrid.querySelectorAll('.btn').forEach((b) => { b.className = 'btn btn-sm btn-outline'; });
            e.currentTarget.className = 'btn btn-sm btn-primary';
          }
        }, m.slice(0, 3));
      ...
      const d1 = h('div');
      d1.appendChild(h('label', { className: 'label' }, 'Ano'));
      const ai = h('input', { className: 'input', type: 'number', value: fd.ano, style: 'text-align:center;font-size:14px;padding:10px' });
      ai.addEventListener('input', (e) => { fd.ano = +e.target.value; });
      d1.appendChild(ai);
      row.appendChild(d1);

      const d2 = h('div');
      d2.appendChild(h('label', { className: 'label' }, 'Máx/dia'));
      const mi = h('input', { className: 'input', type: 'number', value: fd.max_por_dia, style: 'text-align:center;font-size:14px;padding:10px' });
      mi.addEventListener('input', (e) => { fd.max_por_dia = +e.target.value; });
      d2.appendChild(mi);
      row.appendChild(d2);
      ...
      body.appendChild(h('button', {
        className: 'btn btn-primary btn-block',
        style: 'font-size:14px;padding:14px;margin-bottom:8px',
        onClick: async () => {
          const r = await Api.agendaGerar(fd);
          if (r?.success) {
            Toast.show(r.message);
            close();
            sel.setMonth(fd.mes - 1);
            sel.setFullYear(fd.ano);
            draw();
          } else {
            Toast.show(r?.error || 'Erro', 'error');
          }
      ...
      body.appendChild(h('button', {
        ...
        onClick: async () => {
          ...
          const r = await Api.agendaLimpar({ mes: fd.mes, ano: fd.ano });
          Toast.show(r?.message || 'Limpo');
        }
      }, '🗑 Limpar mês'));
    }, '500px');
  }
  ...
  function modalRegioes() {
    showModal('📍 Regiões', async (body) => {
      async function dR() {
        body.innerHTML = '';
        const rgs = await Api.regioes() || [];

        rgs.forEach((r) => {
          const rw = h('div', { style: 'display:flex;align-items:center;gap:8px;padding:10px;border-bottom:1px solid #f1f5f9' });
          ...
          rw.appendChild(h('button', {
            style: 'border:none;background:var(--bg-subtle);border-radius:6px;padding:4px 8px;cursor:pointer;font-size:10px',
            onClick: () => eR(r)
          }, '✏️'));
          body.appendChild(rw);
        });
      ...
      function eR(r) {
        ...
        const fd = {
          nome: r?.nome || '',
          cor: r?.cor || '#2BBCB3',
          dia_semana: r?.diaSemana != null ? String(r.diaSemana) : '',
          bairros: (r?.bairros || []).join(', ')
        };

        const gr = h('div', { className: 'form-grid' });

        [['nome', 'Nome'], ['cor', 'Cor hex']].forEach(([k, l]) => {
          const d = h('div');
          d.appendChild(h('label', { className: 'label' }, l));
          const i = h('input', { className: 'input', value: fd[k] });
          i.addEventListener('input', (e) => { fd[k] = e.target.value; });
          d.appendChild(i);
          gr.appendChild(d);
        });

        const dd = h('div');
        dd.appendChild(h('label', { className: 'label' }, 'Dia'));
        dd.appendChild(buildSelect([['', 'Flex'], ['1', 'Seg'], ['2', 'Ter'], ['3', 'Qua'], ['4', 'Qui'], ['5', 'Sex'], ['6', 'Sáb']], fd.dia_semana, (v) => { fd.dia_semana = v; }));
        gr.appendChild(dd);

        const db = h('div', { style: 'grid-column:1/-1' });
        db.appendChild(h('label', { className: 'label' }, 'Bairros (vírgula)'));
        const bi = h('textarea', { className: 'input', style: 'min-height:50px' });
        bi.textContent = fd.bairros;
        bi.addEventListener('input', (e) => { fd.bairros = e.target.value; });
        db.appendChild(bi);
        gr.appendChild(db);
        ...
        body.appendChild(h('button', {
          ...
          onClick: async () => {
            const d = {
              nome: fd.nome,
              cor: fd.cor,
              dia_semana: fd.dia_semana ? +fd.dia_semana : null,
              bairros: fd.bairros.split(',').map((s) => s.trim()).filter(Boolean)
            };

            const rs = r ? await Api.editarRegiao(r.id, d) : await Api.criarRegiao(d);
            if (rs?.success) {
              Toast.show('Salva');
              window._vittaRegioes = null;
              dR();
            } else {
              Toast.show(rs?.error || 'Erro', 'error');
            }
          }
        }, '💾 Salvar'));
  ...
  async function exportPDF() {
    ...
    Object.values(rg).forEach((g) => {
      ...
      g.items.forEach((it) => {
        const key = it.cliente_id + '-' + it.horario;
        if (seen[key]) {
          seen[key].vacinas.push((it.vacina || '') + (it.dose_numero ? ' D' + it.dose_numero : ''));
        } else {
          seen[key] = {
            ...it,
            vacinas: [(it.vacina || '') + (it.dose_numero ? ' D' + it.dose_numero : '')]
          };
          merged.push(seen[key]);
        }
      });
  ...
  <div class="body">`;
    ...
    Object.entries(rg).forEach(([rn, g]) => {
      ...
      g.merged.forEach((it) => {
        ...
        const stTxt = it.status === 'realizado'
          ? '✅ REALIZADO'
