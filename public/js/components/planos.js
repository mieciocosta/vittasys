...
const t=buildSortableTable([['Cliente','cliente_nome'],['Código','codigo_cliente'],['Plano','nome_plano'],['Faixa',''],['Valor','valor_final'],['Pago',''],['Saldo',''],['Desc%',''],['Progresso',''],['Vendedor','vendedor_nome'],['Status','status_contrato']],f,draw);
const tb=h('tbody');
if(!data.data.length)tb.innerHTML='<tr><td colspan="11" class="empty-state">Nenhum plano</td></tr>';
  ...
  tr.innerHTML=`<td class="fw-600" style="cursor:pointer" onclick="event.stopPropagation();AppState.verCliente(${p.cliente_id})">${esc(p.cliente_nome)}</td><td class="mono text-sm">${esc(p.codigo_cliente||'-')}</td><td class="fw-600">${esc(p.nome_plano)}</td><td class="text-sm">${p.idade_inicio}-${p.idade_fim}m</td><td class="mono fw-600" style="color:#059669">${fmtMoeda(p.valor_final)}</td><td class="mono" style="color:var(--primary)">${fmtMoeda(p.total_pago)}</td><td class="mono" style="color:${p.saldo_pendente>0?'#d97706':'#059669'}">${fmtMoeda(p.saldo_pendente)}</td><td class="text-sm">${p.percentual_desconto>0?p.percentual_desconto+'%':'-'}</td><td><div style="display:flex;align-items:center;gap:6px"><div class="prog-bar" style="width:50px"><div class="prog-fill" style="width:${prog}%;background:${prog===100?'#059669':'var(--primary)'}"></div></div><span class="mono text-sm">${prog}%</span></div></td><td class="text-sm text-muted">${esc(p.vendedor_nome||'-')}</td><td><span class="badge ${p.status_contrato==='ativo'?'badge-green':p.status_contrato==='pendente'?'badge-orange':'badge-gray'}">${p.status_contrato}</span></td>`;
  tb.appendChild(tr);
});
