function renderSidebar(alertCount) {
  ...
  const perfilLabel = {master: '👑 Master', ativos: '⭐ Ativos', espontaneos: '📋 Espontâneos', atendimento: '📞 Atendimento', operador: '🔧 Operador'};
  const allItems = [
  ...
  const items = allItems.filter(it => { if (it.sep) return true; return AppState.temPermissao(it.key); });
  ...
  const logoWrap = h('div', { style: { display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: '6px' } });
  const logoImg = h('img', { src: '/assets/logos/logo-horizontal-white.png', alt: 'Vittalis', style: { height: '32px', objectFit: 'contain', display: 'block', maxWidth: '100%' } });
  logoImg.onerror = function () { this.replaceWith(h('span', { style: { fontSize: '18px', fontWeight: '700', color: '#f1f5f9' } }, '💎 VittaSys')); };
  logoWrap.appendChild(logoImg); brand.appendChild(logoWrap);
  brand.appendChild(h('div', { className: 'sb-brand-sub' }, 'Sistema de Gestão de Vacinação'));
  if (window._vittaEnv && window._vittaEnv !== 'production') { brand.appendChild(h('div', { style: 'background:#f97316;color:white;font-size:9px;font-weight:800;padding:2px 8px;border-radius:4px;text-align:center;margin-top:4px;letter-spacing:1px;text-transform:uppercase' }, '⚠ ' + window._vittaEnv)); }
  nav.appendChild(brand);
  const nw = h('div', { className: 'sb-nav' });
  items.forEach(it => { if (it.sep) { nw.appendChild(h('div', { className: 'sb-sep' }, it.sep)); return; }
