...
r.get('/',async(req,res,next)=>{try{
  ...
  const [totalDoses,totalLotes,vencidos,proxVencer,movsHoje,totalCli,cliAtivos,cliEsp,totalContratos,dosePend,doseAplic,totalRecebido,valorContratos]=await Promise.all([
  ...
  res.json({
    ...
    planos:{total_contratos:totalContratos,doses_pendentes:dosePend,doses_aplicadas:doseAplic},
