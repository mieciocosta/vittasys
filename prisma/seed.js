...
async function main() {
  ...
  for (const u of usuarios) {
    try {
      ...
      uOk++;
    } catch(e) { console.log(`  ⚠️  Usuário já existe ou erro: ${u.nome}`); }
  }
  console.log(`  ✓ ${uOk} usuários verificados`);
  ...
  if (planosExist === 0) {
    ...
    for (const p of planos) { try { await prisma.plano.create({ data: p }); } catch(e) {} }
...
main()
  .catch(e => { console.error('⚠️  Seed com aviso (sistema continua):', e.message); })
